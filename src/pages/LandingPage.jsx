...
const STEPS = [
  {
    icon: RadioTower,
    title: 'Sense',
    text: 'Open hardware measures particles, temperature, and humidity.',
  },
  { icon: Database, title: 'Connect', text: 'Readings flow securely into the Open AIQ backend.' },
  {
    icon: Share2,
    title: 'Open data',
    text: 'Owners choose which readings and locations to publish.',
  },
  {
    icon: Map,
    title: 'See together',
    text: 'Communities turn local measurements into shared awareness.',
  },
]
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        <section className="relative overflow-hidden border-b bg-slate-950 text-white">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 md:py-24 lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:px-8">
            <div>
              <p className="mb-4 text-sm font-medium tracking-[.18em] text-emerald-300 uppercase">
                Open-source air quality network
              </p>
              <h1 className="max-w-xl text-5xl leading-[.95] font-semibold tracking-[-.05em] sm:text-6xl">
                See the air our community breathes.
              </h1>
              <p className="mt-6 max-w-lg text-lg leading-8 text-slate-300">
                Open AIQ connects community-built sensors to a shared, live view of the air around
                us. Ttransparent from device to data.
              </p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Link
                  ...
                  to="/map"
                >
                  Explore the live map <ArrowRight />
                </Link>
                <Link
                  className={cn(
            ...
            <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-white/10 p-2 shadow-2xl shadow-emerald-950/30">
              <PublicMap
                {...map}
                compact
                className="h-[390px] rounded-[1.15rem]"
              />
              <Link
                className={cn(
                  buttonVariants({ variant: 'secondary' }),
                  'absolute right-5 bottom-5 z-10 shadow-lg',
                )}
                to="/map"
                aria-label="Open the full community air map"
              >
        ...
        <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          ...
          <h2 className="mt-2 text-3xl font-semibold tracking-tight">
            Air-quality data you can inspect and improve.
          </h2>
          <div className="mt-10 grid gap-4 md:grid-cols-4">
            {STEPS.map(({ icon: Icon, title, text }, index) => (
              ...
              >
                ...
                <Icon className="mt-6 size-6 text-emerald-600" />
        ...
        <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="rounded-3xl bg-emerald-950 px-6 py-12 text-white sm:px-10 lg:flex lg:items-center lg:justify-between">
            <div>
              <Wind className="size-8 text-emerald-300" />
              <h2 className="mt-5 text-3xl font-semibold">
                Built in the open, for the air we share.
              </h2>
              <p className="mt-3 max-w-2xl text-emerald-100/75">
                Build a sensor, self-host the stack, improve the software, or publish a station.
                Every layer of Open AIQ is open to the community.
              </p>
            </div>
            <div className="mt-8 flex flex-col gap-3 lg:mt-0 lg:ml-8">
              <a
                className={cn(
      ...
      <footer className="border-t py-8 text-center text-sm text-muted-foreground">
        Open AIQ · Open hardware, open software, community data.
      </footer>
    </div>
  )
}
